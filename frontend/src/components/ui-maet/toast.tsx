...
function ToastItem({ toast, onDismiss }: { toast: Toast; onDismiss: () => void }) {
  ...
  const toneClass = {
    ...
    error: 'border-dn/40 text-dn',
    warning: 'border-warn/40 text-warn',
    info: 'border-accent/40 text-accent',
  }[toast.type]

  return (
    <div className={cn('pointer-events-auto rounded border bg-panel p-3', toneClass)} role="status">
      <div className="flex items-start gap-3">
        <Icon className="mt-0.5 h-4 w-4 shrink-0" />
        <div className="min-w-0 flex-1">
          <div className="text-sm font-bold text-text-primary">{toast.title}</div>
          {toast.body && <div className="mt-0.5 text-xs leading-relaxed text-text-muted">{toast.body}</div>}
        </div>
        <button
          type="button"
          onClick={onDismiss}
          aria-label="Dismiss notification"
          className="grid h-8 w-8 shrink-0 place-items-center rounded text-text-muted hover:bg-hover hover:text-text-primary"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
