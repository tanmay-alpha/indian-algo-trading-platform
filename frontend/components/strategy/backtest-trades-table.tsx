  ...
  return (
    <section className="rounded-sm border border-border bg-panel/60">
      <div className="border-b border-border bg-bg/60 px-3 py-2">
        ...
        <div className="text-xs font-mono text-text-faint">Simulated long-only fills</div>
      </div>
      <TableHeader />
      {trades.length === 0 ? (
        <div className="p-3">
          <StrategyEmptyState
            ...
            hint="The selected rules did not produce completed backtest trades."
          />
        </div>
      ) : (
        <div className="max-h-[220px] overflow-auto">
          {trades.map((trade, index) => (
            <div key={`${trade.entry_time}-${index}`} className="grid grid-cols-9 gap-2 border-b border-border/60 px-2 py-1.5 text-xs font-mono text-text-2">
...
function TableHeader() {
  ...
  return (
    <div className="grid grid-cols-9 gap-2 border-b border-border bg-bg px-2 py-1.5 text-xs font-mono uppercase text-text-faint">
