    ...
    <div className="h-full flex flex-col min-h-0">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border bg-panel/60">
        <div className="flex items-center gap-2">
          ...
          {sessionStatus && (
            <span className={cn(
              'px-1.5 py-0.5 rounded-sm border text-xs font-mono font-semibold',
              sessionStatus.is_valid
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="inline-flex items-center gap-1 h-6 px-2 rounded-sm border border-info/30 bg-info/10 text-info text-xs font-mono hover:bg-info/20 disabled:opacity-40"
          >
      ...
      {isUnavailable && (
        <div className="m-3 rounded-sm border border-border bg-panel/40 p-4 flex flex-col items-center gap-2 text-center">
          ...
          {sessionStatus?.last_error && (
            <div className="text-xs font-mono text-down mt-1">{sessionStatus.last_error}</div>
          )}
        </div>
      )}
      ...
      {/* Footer */}
      {lastRefreshed && (
        <div className="px-3 py-1 border-t border-border text-xs font-mono text-text-faint opacity-60 flex items-center justify-between">
          <span className="text-xs opacity-70">READ-ONLY · NO ORDER PLACEMENT</span>
          <span>
...
function OverviewTab({
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-sm border border-border bg-panel/30 p-3 space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-semibold text-text">Historical Data & P&L (Read-only)</span>
          </div>
          <button
            onClick={handleImport}
            disabled={importing || syncing || loading}
            className="inline-flex items-center gap-1 h-5 px-2 rounded-sm border border-info/30 bg-info/10 text-info text-xs font-mono hover:bg-info/20 disabled:opacity-40"
          >
        ...
        <div className="grid grid-cols-2 gap-2 text-xs font-mono">
          <div className="rounded-sm border border-border/40 bg-panel/20 p-2">
            <div className="text-xs uppercase tracking-wider text-text-faint">Last History Import</div>
          ...
          <div className="rounded-sm border border-border/40 bg-panel/20 p-2">
            <div className="text-xs uppercase tracking-wider text-text-faint">Last P&L Calculation</div>
          ...
          <div className="rounded-sm border border-border/40 bg-panel/20 p-2">
            <div className="text-xs uppercase tracking-wider text-text-faint">Total Hist. Trades</div>
            <div className="text-text font-medium mt-0.5">
              {snapshot.total_historical_trades ?? 0}
            </div>
          </div>
          <div className="rounded-sm border border-border/40 bg-panel/20 p-2">
            <div className="text-xs uppercase tracking-wider text-text-faint">Total Hist. Orders</div>
            <div className="text-text font-medium mt-0.5">
      ...
      {/* Source + sync time */}
      <div className="text-xs font-mono text-text-faint opacity-60">
...
function CountStat({ label, value }: { label: string; value: number }) {
  return (
    <div>
      ...
      <div className="text-xs text-text-faint uppercase tracking-wider">{label}</div>
    </div>
  )
}
...
function HoldingsTab({ snapshot }: { snapshot: BrokerAccountSnapshot }) {
  ...
  return (
    <table className="w-full text-xs font-mono">
      <thead>
        <tr className="text-xs uppercase tracking-wider text-text-faint border-b border-border">
...
function TabPill({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={cn(
        'h-5 px-2 rounded-sm text-xs font-mono capitalize font-medium transition-colors',
...
function Empty({ label }: { label: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-8 gap-2">
      ...
      <div className="text-xs font-mono text-text-faint">{label}</div>
    </div>
  )
}
